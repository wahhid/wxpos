...
def enableModule(mod_names, enable=True):
    ...
    pos.config['mod', 'disabled_modules'] = ','.join(disabled_set)
    pos.config.save()
...
def getInstallerInfo(target):
    ...
    if 'install.cfg' not in z.namelist() or [1 for name in z.namelist() if not name.startswith('pos/modules/') and not name.startswith('res/')].count(1)>1:
        return False
...
def installModule(target, replace=False):
    info = getInstallerInfo(target)
    if not info:
        return False
    
    installed = pos.modules.isInstalled(info[0])
    if not replace and installed:
        return False
    elif replace and installed:
        uninstallModule(mod_name, remove_resources=False)
...
def uninstallModule(mod_name, remove_resources=True):
    ...
    shutil.rmtree('./pos/modules/'+mod_name)
    if remove_resources:
...
def exportModule(mod, target, export_source=False):
    ...
    if export_source:
        os.path.walk('pos/modules/'+mod.name, lambda *args: zipdirectory(*args, ignore_hidden=True), z)
    else:
        z.writepy('pos/modules/'+mod.name, 'pos/modules')
